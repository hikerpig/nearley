(function(root, factory) {
    ...
    function Compile(structure, opts) {
        ...
        var result = {
            ...
            customTokens: [], // %tokens
            config: {
                skipUnmatchSymbols: [], // token names that we ignore, usually used for dropping whitespaces
            }, // @config value
            macros: {},
            start: '',
        ...
        for (var i = 0; i < structure.length; i++) {
            ...
            if (productionRule.body) {
                ...
                if (opts.alreadycompiled.indexOf(path) === -1) {
                    ...
                    result.config.skipUnmatchSymbols = result.config.skipUnmatchSymbols.concat(c.config.skipUnmatchSymbols);
            ...
            } else if (productionRule.ignoreSymbol) {
                const resultToken = buildToken('ignore', { token: productionRule.ignoreSymbol }, {})
                result.config.skipUnmatchSymbols.push(resultToken)
            } else {
