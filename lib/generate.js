(function(root, factory) {
    ...
    function serializeParserConfig(parser) {
        var reservedKeys = ["lexer", "preprocessor"];
        var ret = "{";
        var segs = [];
        for (k in parser.config) {
            if (!parser.config.hasOwnProperty(k)) continue;
            if (reservedKeys.indexOf(k) > -1) continue;

            var segV;
            if (k === 'skipUnmatchSymbols') {
                const skipUnmatchSymbolstring = parser.config.skipUnmatchSymbols.map(function(token) {
                    return serializeSymbol(token);
                }).join(',');
                segV = "[" + skipUnmatchSymbolstring + "]";
            } else {
                segV = JSON.stringify(parser.config[k]);
            }
            segs.push(k+ ":" + segV);
        }
        ret += segs.join(',') + "}";
    ...
    generate.js = generate._default = generate.javascript = function (parser, exportName) {
        ...
        output += "  , ParserConfig: " + serializeParserConfig(parser) + "\n";
        output += "}\n";
    ...
    generate.module = generate.esmodule = function (parser, exportName) {
        ...
        output += "let ParserConfig = " + serializeParserConfig(parser) + ";\n";
    ...
    generate.ts = generate.typescript = function (parser, exportName) {
        ...
        output += "\n";
        output += "type NearleyParserConfig = {\n";
        output += "  skipUnmatchSymbols: NearleySymbol[];\n";
        output += "}\n";
        output += "\n";
        ...
        output += "  ParserConfig: NearleyParserConfig;\n";
        output += "};\n";
        output += "\n";
